...
{
    public partial class frmCptSample : Form
    {
        ...
        public frmCptSample()
        {
            ...
            ((PlotChannelTrace)plot.Channels[2]).DataPointClick += TraceClicked;
        }

        void UpdateTempZoneGroup(int i)
        {
            tempZone = tempProfile.TempZones[i];
            dudTemp.Value = tempZone.Temp;
            dudTime.Value = tempZone.Time;
            dudDeviation.Value = tempZone.Deviation;
        }
        ...
        void UpdateDisplay()
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        plot.Channels[0].AddXY(p.Time, p.Temp);
                        plot.Channels[1].AddXY(p.Time, p.Temp + p.Deviation);
                        plot.Channels[2].AddXY(p.Time, p.Temp - p.Deviation);
                    }
        ...
        private void btnAddTempZone_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    Temp = this.dudTemp.Value,
                    Deviation = this.dudDeviation.Value,
        ...
        private void btnDeleteTempZone_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                tempProfile.DelByTime(this.dudTemp.Value);
                UpdateDisplay();
        ...
        void UpdateModelGroup()
        {
            ...
            dudDelayTime.Value = model.DelayTime;
        }

        private void cbModelName_SelectedIndexChanged(object sender, EventArgs e)
        {

            tempProfile = cptModels[cbModelName.SelectedIndex].DefaultProfile;
            UpdateModelGroup();
            UpdateDisplay();
        }
